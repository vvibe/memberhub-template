...
test('visitor, member, and admin states show different screens in both reference sites', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.nav-list').getByRole('button', { name: '後台', exact: true })).toHaveCount(0)
  await openNav(page, '社群')
  await expect(page.getByLabel('社群河道', { exact: true })).toBeVisible()
  await page.getByLabel('撰寫社群貼文').fill('我今天完成了第一版 AI Skill，想請大家幫我看輸出格式。')
  await page.getByRole('button', { name: '發布' }).click()
  await expect(page.getByText('我今天完成了第一版 AI Skill，想請大家幫我看輸出格式。')).toBeVisible()
  await openNav(page, '課程')

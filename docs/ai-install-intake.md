...
7. 是否需要入會問題、管理員審核、檢舉處理或 AutoMod 風險佇列？出版訂閱通訊通常不需要入會審核，除非你要做封閉式會員制。
8. 先不啟用金流可以嗎？我會等前台、InsForge Google 登入、內容、會員流程完成後，再問是否啟用 Portaly 金流、訂閱與發票。
9. 你已經有 InsForge、Portaly Vibe、Email/LINE、發票服務或網域嗎？沒有也可以先跑本機體驗；需要 API key 時，我會引導你去後台取得並放進 `.env.local` 或 secret manager。
```

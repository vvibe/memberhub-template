...
- 已包含 Portaly Vibe MCP 專案設定、InsForge migration、Portaly checkout/webhook Edge Function template、Playwright QA。

### Fork 後需要先準備什麼

- Node `^22.13.0 || ^20.12.0`。
- Production 需要 InsForge 專案與 Portaly Vibe 帳號/key。
...
AI Agent 必須先完成本機 demo、前台、登入、InsForge、Portaly Vibe 產品優化設定，再詢問：
...
- Portaly Vibe：產品優化、MCP、checkout 或方案使用量，依實際方案為準。
...
建議 README 或交付文件都要主動揭露這些項目，避免使用者以為 fork 後完全零成本上線。
...
- Portaly checkout 只能由受信任前端來源呼叫，不能使用 `*` CORS。
- Portaly webhook 要驗證 `x-portaly-signature` 與 timestamp，避免重放或偽造 callback。
...
- 補齊並測試 RLS policies。
- 部署 `insforge/functions/portaly-checkout` 與 `insforge/functions/portaly-webhook`。
- 把 `.env.local` 的正式值放到部署平台 secret manager。
- 用 Portaly test key 跑完整 checkout、callback、訂閱與發票任務流程。
...
- webhook 失敗：檢查 `PORTALY_CALLBACK_SECRET`、timestamp、signature 與 callback URL。
...
- The repo includes project-scoped Portaly Vibe MCP config, an InsForge migration, Portaly checkout/webhook Edge Function templates, and Playwright QA.
...
The AI agent must finish the local demo, frontend, login, InsForge, and Portaly Vibe product optimization setup first, then ask whether to enable payments, subscription plans, and invoice flow.

### Possible costs

- InsForge usage: Auth, Postgres, Edge Functions, Storage.
- Portaly Vibe usage: product optimization, MCP, checkout, or plan features depending on the plan.
...
- Portaly checkout should only accept trusted frontend origins.
- Portaly webhooks must verify `x-portaly-signature` and timestamp.
...
- Deploy `insforge/functions/portaly-checkout` and `insforge/functions/portaly-webhook`.
- Move real env values into the deployment secret manager.
- Test checkout, callback, subscription, and invoice task flow with a Portaly test key.

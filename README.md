...
你可以先不接任何外部服務，直接在本機跑 demo。等確認要正式使用，再接 InsForge 做登入與資料庫，並用 Portaly Vibe MCP 讓 AI agent 協助檢查與優化產品設定。

## 注意事項

- 本機 demo 不需要 API key；正式部署才需要設定 InsForge 與 Portaly Vibe MCP token。
...
目前 demo 不需要任何 key 就能跑；正式部署才需要 InsForge 設定與 Portaly Vibe MCP token。
...
如果你只是想體驗 demo，可以跳過這段。若要正式部署，通常需要：

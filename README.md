...
你可以先不接任何外部服務，直接在本機體驗完整流程。等確認要正式使用，再接 InsForge 做登入與資料庫，並用 Portaly Vibe MCP 讓 AI agent 協助檢查與優化產品設定。

## 注意事項

- 本機體驗不需要 API key；正式部署才需要設定 InsForge 與 Portaly Vibe MCP token。
- 如果要正式收款，才需要另外設定金流、訂閱方案與發票流程。
- 可能產生成本的項目包含：部署平台、網域、InsForge、Portaly Vibe、金流手續費、Email/LINE 通知、發票或電子發票服務。
- 請不要把真正的 API key、MCP token 或 callback secret commit 到 GitHub。
...
- Skool 版本：https://memberhub-coral.vercel.app/?case=skills-school&view=join
- Substack 版本：https://memberhub-coral.vercel.app/?case=superstake&view=blog
- Vercel project: `memberhub`
- 目前包含兩個正式案例：`Skills School 職能加速社群` 與 `SuperStake 策略通訊`。

## 這是什麼

MemberHub 是一個開源的會員平台範本，目標是讓創作者、教練、老師、顧問和社群經營者，可以用比較短的時間建立自己的會員網站。

這不是只有文件或設計稿。這個專案已包含可執行的 Vite + React 服務、兩組可切換正式案例、付費牆、Newsletter、推薦贈閱、全站搜尋、會員目錄、課程資源、社群審核、打卡、活動、會員自助與可編輯營運後台。下載後可以先用本機資料跑完整預覽，再依需求接上 InsForge 和 Portaly Vibe MCP，變成可私有化部署的正式服務。
...
正式部署時，把 `src/lib/store.ts` 的 localStorage state 替換為 InsForge CRUD；金流、訂閱與發票流程則等核心功能完成後，再依照 README 的 AI Agent 順序詢問使用者是否啟用。
...
src/
  ...
  data/presets.ts         # 可切換案例：Skills School、SuperStake
...
MCP server 名稱是 `portaly-vibe`，HTTP URL 是 `https://mcp.portaly.ai`，Authorization header 範本是 `Bearer <YOUR_TOKEN>`。

你只需要申請 Portaly Vibe MCP token，並把 `<YOUR_TOKEN>` 換成自己的 token。不要把真 token commit 到 GitHub。更多說明見 `docs/mcp-setup.md`。
...
`npm ci` 會一起安裝 `@insforge/sdk`、`@insforge/cli` 與 Playwright QA 工具。InsForge migration、browser SDK client、Portaly Vibe MCP 設定、AI 安裝問答與 launch checklist 都已放在 repo 內。

金流、訂閱方案與發票屬於選配功能。若你要啟用，請先用測試模式確認完整流程，再切換成正式收款。
...
6. 先讓本機資料可用，並確認 Skills School 與 SuperStake 兩個案例可以切換。
...
如果需要，我會先用測試模式設定月繳、年繳、終身方案、付款狀態同步與發票任務記錄；確認流程無誤後，再由你決定是否切換成正式收款。
```
...
1. `skills-school`: 課程、社群、打卡與加入會員流程，適合 Skool/School 類型的會員學習社群。
2. `superstake`: 對外公開部落格、付費文章、Email Newsletter 與訂閱方案，適合 Substack 類型的內容訂閱服務。

## Fork 後如何客製

1. 先用後台「基礎編輯」調整品牌、首頁文案、方案、文章付費狀態、課程標題與 Newsletter 發送設定。
...
- Substack 型案例可以閱讀公開文章、看到付費文章牆、訂閱付費文章與建立文章電子報。
- 管理員可以編輯網站名稱、首頁文案、方案、文章、課程標題與 Newsletter 發送設定。
